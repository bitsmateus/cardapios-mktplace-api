...
public interface CardapiosRepository extends JpaRepository<Cardapio, Integer> {
	...
	@Query(value = "SELECT Count(c) " + "FROM Cardapio c " + "WHERE c.restaurante.id = :id ")
	public Long ContarPor(Integer id);

	@Query(value = "SELECT c " + "FROM Cardapio c " + "JOIN FETCH c.restaurante r " + "JOIN FETCH c.opcoes oc "
			+ "JOIN FETCH oc.opcao o " + "JOIN FETCH oc.secao s " + "WHERE c.restaurante = :restaurante "
			+ "AND o.status = 'A' " + "ORDER BY oc.recomendado DESC, o.nome ", countQuery = "SELECT Count(c) "
					+ "FROM Cardapio c " + "WHERE c.restaurante = :restaurante ")
	public Page<Cardapio> ListarPorRestaurante(Restaurante restaurante, Pageable paginacao);

	@Query(value = "SELECT c " + "FROM Cardapio c " + "JOIN FETCH c.restaurante r " + "JOIN FETCH c.opcoes oc "
			+ "JOIN FETCH oc.opcao o " + "JOIN FETCH oc.secao s " + "WHERE c.id = :id " + "AND o.status = 'A' "
